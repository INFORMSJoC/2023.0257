...
data=xlsread('oreder priority data');
Y=data(:,12);             
X=data(:,2:11);
N=length(Y);
p=size(X,2);
...
n2s=8;                         
n1=25;                       
if n1==15; Ks=floor([2,5,n1/2,n1]);  end
if n1==20; Ks=floor([2,5,n1/2,n1]);  end
if n1==25; Ks=floor([2,5,n1/2,n1]);  end
if n1==28; Ks=floor([2,5,n1/2,n1]);  end
kn=length(Ks);
cons=ones(n1,1);
...
%construct order of variables
effect_col=[];
for corrcoef_i = 1:size(x_initial,2) 
effect_matrix = corrcoef(y,x_initial(:,corrcoef_i));
effect_col(corrcoef_i) = abs(effect_matrix(2,1));
end
[numeric,order]=sort(effect_col,2,'descend');
...
for k=1:kn
    if k~=kn
        K=Ks(k);
%         [w_k,v_k,wplma,vplma,losspl]=plma_kcv_linear_approximate_lambda_CV_all(x,y,n1,K,M,w0,options);
%         [w_k,v_k,wplma,vplma,losspl]=plma_kcv_linear_approximate_CV_all_consistence_Annals(x,y,n,K,M,w0,options);
        [w_k,v_k,wplma,vplma,losspl]=plma_kcv_linear_approximate_lambda_CV_all_Annals(x,y,n1,K,M,w0,options);
        w_ks(:,k)=w_k;  
        v_ks(k)=v_k;  
        y_ks(:,k)=yy'*w_k;
        y_ks_in(:,k)=yin'*w_k;
...
% % % % %         ppl_Annals_4_pl_in(k)=pairwise_loss_approximate_Annals_4(length(y),y,yin',wplma'); 
    ...
    elseif k==kn
        K=Ks(k);
        [w_k,v_k]=kcv_nls_linear(x,y,n1,K,M,w0,options);
        w_ks(:,k)=w_k;  
        v_ks(k)=v_k;
        y_ks(:,k)=yy'*w_k;
        y_ks_in(:,k)=yin'*w_k;
...
xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\OP_exapmle_mean.xlsx',ppl_cn(:,[1:8,10,15:17])./ppl_cn(:,16),1,'A3'); 
xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\OP_exapmle_mean.xlsx',ppl_Annals_2_cn(:,[1:8,10,15:17])./ppl_Annals_2_cn(:,16),1,'A7'); 
xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\OP_exapmle_std.xlsx',ppl_cn_sd(:,[1:8,10,15:17])./ppl_cn_sd(:,16),1,'A3'); 
xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\OP_exapmle_std.xlsx',ppl_Annals_2_cn_sd(:,[1:8,10,15:17])./ppl_Annals_2_cn_sd(:,16),1,'A7'); 
xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\OP_exapmle_com_mean.xlsx',ppl_cn(:,10),1,'A3'); 
xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\OP_exapmle_com_mean.xlsx',ppl_Annals_2_cn(:,10),1,'A7');
xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\OP_exapmle_com_std.xlsx',ppl_cn_sd(:,10),1,'A3'); 
xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\OP_exapmle_com_std.xlsx',ppl_Annals_2_cn_sd(:,10),1,'A7'); 