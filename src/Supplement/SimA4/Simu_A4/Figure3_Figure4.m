...
sig_x=rho*(ones(q,q)-eye(q))+eye(q);
% sig_x=eye(q);
...
kn=length(Ks);
r1_cn=zeros(cn,2*kn+3);
r1_cn_in=zeros(cn,2*kn+3);
r2_cn=zeros(cn,2*kn+3);
r2_cn_in=zeros(cn,2*kn+3);
w_cn=zeros(cn,kn+1);
w_cn_pl=zeros(cn,kn);
...
rank_score_cn=zeros(cn,2*kn+3);
rank_score_cn_in=zeros(cn,2*kn+3);
ppl_cn=zeros(cn,2*kn+7);
ppl_cn_in=zeros(cn,2*kn+7);
% % % % % ppl_Annals_1_cn=zeros(cn,2*kn+7);
% % % % % ppl_Annals_1_cn_in=zeros(cn,2*kn+7);
ppl_Annals_2_cn=zeros(cn,2*kn+7);
ppl_Annals_2_cn_in=zeros(cn,2*kn+7);
% % % % % ppl_Annals_3_cn=zeros(cn,2*kn+7);
% % % % % ppl_Annals_3_cn_in=zeros(cn,2*kn+7);
% % % % % ppl_Annals_4_cn=zeros(cn,2*kn+7);
% % % % % ppl_Annals_4_cn_in=zeros(cn,2*kn+7);

for c=1:cn  
beta=[1;1;1;cs(c);zeros(r,1)];
r1s=zeros(S,2*kn+3);
r1s_in=zeros(S,2*kn+3);
r2s=zeros(S,2*kn+3);
r2s_in=zeros(S,2*kn+3);
ws_cv=zeros(S,kn+1);
wpl_cv=zeros(S,kn);
...
ppl=zeros(S,2*kn+7);
ppl_in=zeros(S,2*kn+7);
% % % % % ppl_Annals_1=zeros(S,2*kn+7);
% % % % % ppl_Annals_1_in=zeros(S,2*kn+7);
ppl_Annals_2=zeros(S,2*kn+7);
ppl_Annals_2_in=zeros(S,2*kn+7);
% % % % % ppl_Annals_3=zeros(S,2*kn+7);
% % % % % ppl_Annals_3_in=zeros(S,2*kn+7);
% % % % % ppl_Annals_4=zeros(S,2*kn+7);
% % % % % ppl_Annals_4_in=zeros(S,2*kn+7);

parfor_progress(S);
parfor s=1:S
if floor(s/50)*50==s; disp([n,c,s/50]); end
% DGP
X=mvnrnd(zeros(1,q),sig_x,n+10);           % test number
if V==1; sig_e=1; end            
if V==2; sig_e=0.5+0.5*(X(:,p).^2);  end 
e=sqrt(sig_e).*randn(n+10,1);              % test number
Y=X*beta+e;
...
% Prediciton Risk
ys=[ye,y_kmin,y_plmin,y_pl_loss_min,y_ks,y_pl];
r1s(s,:)=mean(ys-yT);
r2s(s,:)=mean(ys-y0);
[~,I0]=sort(y0,'descend');
[~,I]=sort(ys,'descend');
...
% Risk in sample
ys_in=[ye_in,y_kmin_in,y_plmin_in,y_pl_loss_min_in,y_ks_in,y_pl_in];
...
ppl_y3=pairwise_loss(length(y0),y0,y3,1);  
ppl_y4=pairwise_loss(length(y0),y0,y4,1);
ppl_ye=pairwise_loss(length(y0),y0,yy',w0');
...
ppl(s,:)=[ppl_y1,ppl_y2,ppl_y3,ppl_y4,ppl_ye,ppl_ykmin,ppl_yplmin,ppl_loss_yplmin,ppl_ks,ppl_pl];
...
ppl_y3_in=pairwise_loss(length(y),y,y3_in,1);
ppl_y4_in=pairwise_loss(length(y),y,y4_in,1);
ppl_ye_in=pairwise_loss(length(y),y,yin',w0');
...
ppl_in(s,:)=[ppl_y1_in,ppl_y2_in,ppl_y3_in,ppl_y4_in,ppl_ye_in,ppl_ykmin_in,ppl_yplmin_in,ppl_loss_yplmin_in,ppl_ks_in,ppl_pl_in];

% % % % % % Prediciton Pairwise loss Annals 1
% % % % % ppl_Annals_1_ye=pairwise_loss_approximate_Annals_1(length(y0),y0,yy',w0');
...
% % % % % ppl_Annals_1(s,:)=[ppl_Annals_1_ye,ppl_Annals_1_ykmin,ppl_Annals_1_yplmin,ppl_loss_Annals_1_yplmin,ppl_Annals_1_ks,ppl_Annals_1_pl];

% % % % % % % % % Pairwise loss in sample Annals approximate 1
% % % % % ppl_Annals_1_ye_in=pairwise_loss_approximate_Annals_1(length(y),y,yin',w0');
...
ppl_Annals_2_y3=pairwise_loss_approximate_Annals_2(length(y0),y0,y3,1);
ppl_Annals_2_y4=pairwise_loss_approximate_Annals_2(length(y0),y0,y4,1);
ppl_Annals_2_ye=pairwise_loss_approximate_Annals_2(length(y0),y0,yy',w0');
...
ppl_Annals_2(s,:)=[ppl_Annals_2_y1,ppl_Annals_2_y2,ppl_Annals_2_y3,ppl_Annals_2_y4,ppl_Annals_2_ye,ppl_Annals_2_ykmin,ppl_Annals_2_yplmin,ppl_loss_Annals_2_yplmin,ppl_Annals_2_ks,ppl_Annals_2_pl];
...
ppl_Annals_2_y3_in=pairwise_loss_approximate_Annals_2(length(y),y,y3_in,1);
ppl_Annals_2_y4_in=pairwise_loss_approximate_Annals_2(length(y),y,y4_in,1);
ppl_Annals_2_ye_in=pairwise_loss_approximate_Annals_2(length(y),y,yin',w0');
...
ppl_Annals_2_in(s,:)=[ppl_Annals_2_y1_in,ppl_Annals_2_y2_in,ppl_Annals_2_y3_in,ppl_Annals_2_y4_in,ppl_Annals_2_ye_in,ppl_Annals_2_ykmin_in,ppl_Annals_2_yplmin_in,ppl_loss_Annals_2_yplmin_in,ppl_Annals_2_ks_in,ppl_Annals_2_pl_in];

% % % % % % Prediciton Pairwise loss Annals 3
% % % % % ppl_Annals_3_ye=pairwise_loss_approximate_Annals_3(length(y0),y0,yy',w0');
...
% % % % % ppl_Annals_3_ye_in=pairwise_loss_approximate_Annals_3(length(y),y,yin',w0');
...
% % % % % ppl_Annals_3_in(s,:)=[ppl_Annals_3_ye_in,ppl_Annals_3_ykmin_in,ppl_Annals_3_yplmin_in,ppl_loss_Annals_3_yplmin_in,ppl_Annals_3_ks_in,ppl_Annals_3_pl_in];

% % % % % % Prediciton Pairwise loss Annals 4
% % % % % ppl_Annals_4_ye=pairwise_loss_approximate_Annals_4(length(y0),y0,yy',w0');
...
% % % % % ppl_Annals_4(s,:)=[ppl_Annals_4_ye,ppl_Annals_4_ykmin,ppl_Annals_4_yplmin,ppl_loss_Annals_4_yplmin,ppl_Annals_4_ks,ppl_Annals_4_pl];
% % % % % 
% % % % % % % % % Pairwise loss in sample Annals approximate 4
% % % % % ppl_Annals_4_ye_in=pairwise_loss_approximate_Annals_4(length(y),y,yin',w0');
...
% % % % % ppl_Annals_4_in(s,:)=[ppl_Annals_4_ye_in,ppl_Annals_4_ykmin_in,ppl_Annals_4_yplmin_in,ppl_loss_Annals_4_yplmin_in,ppl_Annals_4_ks_in,ppl_Annals_4_pl_in];
...
if jj==1; 
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA4_PL50.xlsx',[ppl_Annals_2_cn(:,[1:4,8,19,20,24,27])./ppl_Annals_2_cn(:,20);ppl_cn(:,[1:4,8,19,20,24,27])./ppl_cn(:,20)]); 
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA4_w50.xlsx',w(:,[4,15,16,20,23])); 
end
if jj==2; 
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA4_PL.xlsx',[ppl_Annals_2_cn(:,[1:4,8,19:21,27])./ppl_Annals_2_cn(:,20);ppl_cn(:,[1:4,8,19:21,27])./ppl_cn(:,20)],1,'A1'); 
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA4_w.xlsx',w(:,[4,15:17,23]),1,'A1'); 
end
if jj==3; 
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA4_PL.xlsx',[ppl_Annals_2_cn(:,[1:4,8,19:21,27])./ppl_Annals_2_cn(:,20);ppl_cn(:,[1:4,8,19:21,27])./ppl_cn(:,20)],1,'A25'); 
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA4_w.xlsx',w(:,[4,15:17,23]),1,'A14'); 
end
if jj==4; 
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA4_PL.xlsx',[ppl_Annals_2_cn(:,[1:4,8,19:21,27])./ppl_Annals_2_cn(:,20);ppl_cn(:,[1:4,8,19:21,27])./ppl_cn(:,20)],1,'A49'); 
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA4_w.xlsx',w(:,[4,15:17,23]),1,'A27'); 
end
if jj==5; 
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA4_PL.xlsx',[ppl_Annals_2_cn(:,[1:4,8,19:21,27])./ppl_Annals_2_cn(:,20);ppl_cn(:,[1:4,8,19:21,27])./ppl_cn(:,20)],1,'A73'); 
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA4_w.xlsx',w(:,[4,15:17,23]),1,'A40'); 
end
