...
x=X(1:n,:);                                  
y=Y(1:n);  
t1=(x(:,1:5)'*x(:,1:5))\(x(:,1:5)'*log(y));
beta1=nlinfit(x(:,1:5),y,myfun,t1);
% t0=(x'*x)\(x'*y);
t0=(x'*x)\(x'*log(y));
beta2=nlinfit(x,y,myfun,t0);
x0=X(n+1:n+10,:);                           
y0=Y(n+1:n+10);                             
y1=myfun(beta1,x0(:,1:5));                                
y2=myfun(beta2,x0);                      
y1_in=myfun(beta1,x(:,1:5));                               
y2_in=myfun(beta2,x);                        
yy=[y1,y2];                                 
yin=[y1_in,y2_in];
y3=yy*w0;                                    
...
for k=1:kn                                                                
    ...
    if k~=kn
        K=Ks(k);                                                           
        [w_k,v_k,wplma,vplma,losspl]=plma_kcv_mix_approximate_lambda_CV_all_Annals_log(x,y,n,K,M,t1,t0,w0,myfun,options);
...
% % % % %         ppl_Annals_4_pl_in(k)=pairwise_loss_approximate_Annals_4(length(y),y,yin,wplma');  
    ...
    elseif k==kn 
        K=Ks(k);
        [w_k,v_k]=kcv_mix_log(x,y,n,K,M,t1,t0,w0,myfun,options);
        w_ks(k,:)=w_k;  
        v_ks(k)=v_k;
        y_ks(k,:)=yy*w_k;
        y_ks_in(k,:)=yin*w_k;
...
if jj==2;
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA5_comprae.xlsx',[ppl_Annals_2_cn(:,6);ppl_cn(:,6)],1,'B1'); 
end         
if jj==3; 
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA5_comprae.xlsx',[ppl_Annals_2_cn(:,6);ppl_cn(:,6)],1,'B25');     
end   
if jj==4; 
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA5_comprae.xlsx',[ppl_Annals_2_cn(:,6);ppl_cn(:,6)],1,'B49');     
end     
if jj==5; 
    xlswrite('C:\Users\dell\Desktop\2023.0257\scr\output\SimA5_comprae.xlsx',[ppl_Annals_2_cn(:,6);ppl_cn(:,6)],1,'B73');         
end 
