function [w_kcv,v_kcv,wplma,vplma,losspl] = plma_kcv_mix_approximate_lambda_CV_all_Annals(x,y,n,K,M,t0,w0,myfun,options)
...
for k=1:K                                                     %进行折交叉验证(K组)
    ...
    p_1(idx_k0)=x_k0*b_k1;                                    % 第一个模型的预测值
...
plfun=@(w)CV_pl_mix_Annals(x,y,K,idx,n0,n_K,M,t0,w,myfun);            % 连续情形Annals近似方案1和2
Aeq=ones(1,M);
beq=1;
lb=zeros(1,M);
ub=ones(1,M);
...
losspl=CV_pl_mix_real(x,y,K,idx,n0,n_K,M,t0,w_pl',myfun);             % 连续情形(真实CV)  
