function [w_kcv,v_kcv,wplma,vplma,losspl] = plma_kcv_mix_approximate_lambda_CV_all_Annals_log(x,y,n,K,M,t1,t0,w0,myfun,options)
...
%     b_k1=(x_k'*x_k)\(x_k'*y_k);                             %第一个模型拟合的结果
    ...
    x_k0=x(idx_k0,:);
    y_k0=y(idx_k0);
    e_1(idx_k0)=y_k0-myfun(b_k1,x_k0(:,1:5));                 % 第一个模型的预测误差(K折CV的测试集)
    e_2(idx_k0)=y_k0-myfun(b_k2,x_k0);                        % 第二个模型的预测误差(K折CV的测试集)
    p_1(idx_k0)=myfun(b_k1,x_k0(:,1:5));                      % 第一个模型的预测值
    p_2(idx_k0)=myfun(b_k2,x_k0);                             % 第二个模型的预测值
end
ee=[e_1,e_2];
a1=ee'*ee;
...
plfun=@(w)CV_pl_mix_Annals_log(x,y,K,idx,n0,n_K,M,t1,t0,w,myfun);            % 连续情形Annals近似方案1和2
Aeq=ones(1,M);
beq=1;
lb=zeros(1,M);
ub=ones(1,M);
...
losspl=CV_pl_mix_real_log(x,y,K,idx,n0,n_K,M,t1,t0,w_pl',myfun);             % 连续情形(真实CV)  
