function [w_kcv,v_kcv,wplma,vplma,losspl] = plma_kcv_linear_approximate_lambda_CV_all_Annals_survival(x,y,n,K,M,w0,options)
...
for k=1:K
    ...
    y_k0=y(idx_k0);    
    for m=1:M
        x_km=x_k(:,10*(m-1)+1:10*m);       
        x_k0m=x_k0(:,10*(m-1)+1:10*m);     
...
plfun=@(w)CV_pl_sim2_linear_Annals_survival(x,y,K,idx,n0,n_K,M,w);
Aeq=ones(1,M);
beq=1;
lb=zeros(1,M);
ub=ones(1,M);
...
losspl=CV_pl_sim2_linear_real_survival(x,y,K,idx,n0,n_K,M,w_pl');
